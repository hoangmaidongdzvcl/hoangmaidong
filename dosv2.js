 ...
 if (cluster.isMaster) {
  ...
  `.white);
   ...
   (async () => {
    ...
    const time = 'exampleTime';      
    const rate = 'exampleRate';      
    const thread = 'exampleThread';  
    const proxyFile = 'exampleProxyFile';
    


console.log(`      ╚╦════════════════════════════════╦╝ `.brightYellow);
console.log(` ╔═════╩════════════════════════════════╩═════╗   `.brightYellow)
console.log(`            Attack Successfully Send        `)
...
console.log(`   BOTNET    : `.brightYellow + (Math.floor(Math.random() * (8359 - 7500 + 1)) + 7500));
console.log(`   Owen      : `.brightYellow + "Satoru Hatoki");
console.log(` ╚════════════════════════════════════════════╝`.brightYellow);
console.log(` HTTP-FLOOD | HTTP 1/1 CUSTOM High RQ/S`.red);
    ...
    const restartScript = () => {
        ...
        console.log('[>] Restarting the script'.green, RESTART_DELAY, 'ms...'.green);
        setTimeout(() => {
    ...
    const handleRAMUsage = () => {
        ...
        if (ramPercentage >= MAX_RAM_PERCENTAGE) {
            console.log('[!] Maximum RAM usage:'.green, ramPercentage.toFixed(2), '%'.green);
            restartScript();
        }
    };
